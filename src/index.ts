...
import { EsaClient, type Post } from "esa-api-client";
import { z } from "zod";

if (!process.env.ESA_API_TOKEN || !process.env.ESA_TEAM_NAME) {
	console.error(
		"Please set ESA_API_TOKEN and ESA_TEAM_NAME environment variables.",
	);
	process.exit(1);
}
...
server.tool(
	...
	"Search for posts within esa",
	{
		searchParams: z
			.string()
			.describe("Query parameters for searching within esa"),
	},
	async ({ searchParams }) => {
		const posts = await esa.getPosts({
			q: encodeURI(searchParams.replace(" ", "+")),
		});
		return {
			content: posts.posts.map((post) => formatPost(post)),
		};
	},
);
...
server.tool(
	"create_esa_post",
	"Create a new post in esa",
	{
		title: z.string().describe("Title of the post"),
		body: z.string().describe("Body of the post with markdown"),
		tags: z.array(z.string()).describe("Post tags"),
		category: z.string().describe("Post category"),
	},
	async ({ title, body, tags, category }) => {
